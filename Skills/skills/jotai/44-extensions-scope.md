...
Instead of specifying the `store` option, `<ScopeProvider>` lets you reuse the _same_ atoms in different parts of the React tree **without sharing state** while still being able to read other atoms from the parent store.
